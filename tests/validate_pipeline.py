...
sys.path.insert(
    0, os.path.join(os.path.dirname(__file__), "..", "..", "hifisampler-git")
)
...
try:
    ...
    fb_default = librosa.filters.mel(
        sr=44100, n_fft=2048, n_mels=128, fmin=40.0, fmax=16000.0
    )
    fb_htk = librosa.filters.mel(
...
print(f"  x0 = {x0:.4f}")
print(
    f"  For b=-1.0: filter[0]={(-(-1.0) / x0) * 0 + (-1.0):.4f}, filter[fft_bin-1]={(-(-1.0) / x0) * (fft_bin - 1) + (-1.0):.4f}"
)
...
print(
    f"  For 1s audio: ~{n_mel_frames} mel frames at origin_hop_size={origin_hop_size}"
)
