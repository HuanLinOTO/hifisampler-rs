...
"""
import sys, os
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)) + '/../../hifisampler-git')
...
print(f"mel_raw (before DRC): {mel_raw.shape}, min={mel_raw.min():.6f}, max={mel_raw.max():.6f}")

mel_drc = dynamic_range_compression_torch(mel_raw).squeeze(0)
print(f"mel_drc (after DRC): {mel_drc.shape}, min={mel_drc.min():.6f}, max={mel_drc.max():.6f}")
...
print(f"  sum per band: min={mel_fb.sum(axis=1).min():.6f}, max={mel_fb.sum(axis=1).max():.6f}")
np.save(out_dir / "mel_filterbank.npy", mel_fb)

# Check if librosa uses HTK or Slaney by default
mel_fb_htk = librosa_mel_fn(sr=44100, n_fft=2048, n_mels=128, fmin=40.0, fmax=16000.0, htk=True)
print(f"Mel filterbank (HTK): differs from default? {not np.allclose(mel_fb, mel_fb_htk)}")
