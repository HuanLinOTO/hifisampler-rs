...
_HIFI_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), '..', 'hifisampler')
...
def compare_hifigan():
    ...
    np.random.seed(42)
    n_frames = 100
    mel_nchw = np.random.randn(1, 128, n_frames).astype(np.float32) * 0.1  # [B, mel, T] for pytorch
    ...
    logging.info(f"PyTorch output shape: {pt_wav.shape}, time: {(t1-t0)*1000:.1f}ms")
    ...
    logging.info(f"SNR (dB): {20*np.log10(pt_rms / (np.sqrt(np.mean(diff**2)) + 1e-10)):.1f}")
...
def compare_hnsep():
    ...
    if not os.path.exists(onnx_path):
        logging.warning("HN-SEP ONNX model not found. Run convert_hnsep_to_onnx.py first.")
        return False
    ...
    # Load PyTorch model
    model = CascadedNet(
        cfg['n_fft'], cfg['hop_length'], cfg['n_out'], cfg['n_out_lstm'],
        is_complex=True, is_mono=cfg['is_mono'], fixed_length=True
    )
    ...
    complex_input = torch.complex(torch.from_numpy(real), torch.from_numpy(imag))  # [1, 1, freq, time] complex
    ...
    logging.info(f"PyTorch output shape: {pt_mask.shape}, time: {(t1-t0)*1000:.1f}ms")
...
def test_full_pipeline():
    ...
    mel = np.random.randn(1, n_frames, 128).astype(np.float32) * 0.1  # [B, T, mel] for ONNX
    ...
    wav = out[0].squeeze()
    logging.info(f"HiFi-GAN output: {len(wav)} samples, {len(wav)/sr:.3f}s, time: {(t1-t0)*1000:.1f}ms")
    ...
    if os.path.exists(hnsep_path):
        ...
        logging.info(f"HN-SEP output shape: {mask[0].shape}, time: {(t1-t0)*1000:.1f}ms")
